  ...
  try {
    ...
    await getPool().query(`
      CREATE TABLE IF NOT EXISTS organization_contacts (
        ...
        name VARCHAR(255) NOT NULL,
        role VARCHAR(100),           -- 'Partner', 'Associate', 'Paralegal', 'Nurse', 'Adjuster', etc.
...
async function getOrganizationContext(phoneNumber, agentId) {
  ...
  try {
    const contactsResult = await getPool().query(`
      SELECT name, role, email, direct_phone, fax, preferred_language, total_calls, last_call_date
 ...
 * @param {number} organizationId - Organization ID
 * @param {object} contactData - Contact info {name, role, email, phone, fax}
...
async function getOrCreateOrganizationContact(organizationId, contactData) {
  ...
  try {
    ...
    if (existing) {
      ...
      await getPool().query(`
        ...
        SET
          ...
          direct_phone = COALESCE(NULLIF($3, ''), direct_phone),
          email = COALESCE(NULLIF($4, ''), email),
          fax = COALESCE(NULLIF($5, ''), fax),
          preferred_language = COALESCE(NULLIF($6, ''), preferred_language),
          total_calls = total_calls + 1,
          last_call_date = NOW(),
          updated_at = NOW()
        WHERE id = $1
    ...
    // Create new contact
    const result = await getPool().query(`
      INSERT INTO organization_contacts (organization_id, name, role, direct_phone, email, fax, preferred_language)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      ON CONFLICT (organization_id, LOWER(name)) DO UPDATE SET
        role = COALESCE(NULLIF(EXCLUDED.role, ''), organization_contacts.role),
