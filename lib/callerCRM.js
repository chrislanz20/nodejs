...
async function initializeCallerCRM() {
  try {
    ...
    // Create indexes for callers
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_callers_phone ON callers(phone_number)`);
...
async function lookupCaller(phoneNumber) {
  ...
  try {
    // Find caller by any phone variant
 ...
 */
async function getCallerContext(phoneNumber) {
  const profile = await lookupCaller(phoneNumber);
...
/**
 * Create or get a caller record
 * @param {string} phoneNumber - The phone number
 * @param {string} callId - The Retell call ID
 * @returns {object} Caller record
 */
async function getOrCreateCaller(phoneNumber, callId = null) {
  if (!phoneNumber) return null;
  ...
  try {
    ...
    const existing = await lookupCaller(phoneNumber);
    if (existing) {
    ...
    const result = await pool.query(`
      INSERT INTO callers (phone_number)
      VALUES ($1)
      RETURNING *
    `, [normalizedPhone]);
