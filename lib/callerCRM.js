...
async function initializeCallerCRM() {
  try {
    ...
    // CONTACT-CLIENT ASSOCIATIONS TABLE
    // Tracks which professional contacts have called about which CourtLaw clients
    // ============================================
    await getPool().query(`
      CREATE TABLE IF NOT EXISTS contact_client_associations (
        id SERIAL PRIMARY KEY,
        contact_id INTEGER NOT NULL REFERENCES organization_contacts(id) ON DELETE CASCADE,
        client_caller_id INTEGER NOT NULL REFERENCES callers(id) ON DELETE CASCADE,
        organization_id INTEGER REFERENCES organizations(id) ON DELETE SET NULL,
        client_name VARCHAR(255),           -- Cached client name for easy display
        relationship_type VARCHAR(50),       -- 'represents', 'adjusting_claim', 'treating', etc.
        first_call_date TIMESTAMP DEFAULT NOW(),
        last_call_date TIMESTAMP DEFAULT NOW(),
        total_calls INTEGER DEFAULT 1,
        notes TEXT,
        created_at TIMESTAMP DEFAULT NOW(),
        updated_at TIMESTAMP DEFAULT NOW(),
        UNIQUE(contact_id, client_caller_id)  -- One association per contact-client pair
      )
    `);

    // Create indexes for contact_client_associations
    await getPool().query(`CREATE INDEX IF NOT EXISTS idx_cca_contact ON contact_client_associations(contact_id)`);
    await getPool().query(`CREATE INDEX IF NOT EXISTS idx_cca_client ON contact_client_associations(client_caller_id)`);
    await getPool().query(`CREATE INDEX IF NOT EXISTS idx_cca_org ON contact_client_associations(organization_id)`);

    console.log('✅ Caller CRM tables initialized (including organizations, contacts & associations)');
    return true;
  } catch (error) {
...
async function getCallerContext(phoneNumber, agentId) {
  ...
  if (profile.name) {
    ...
    // Check if we only have first name (no last name)
    const nameParts = profile.name.trim().split(/\s+/);
    if (nameParts.length === 1) {
      // Only first name on file - need to ask for last name
...
async function updateCallerFromCallData(callerId, data, callId) {
  ...
  // Handle last_name only - combine with existing first name
  if (data.last_name && !data.first_name && !data.name) {
    try {
      ...
      if (existingName.rows.length > 0) {
        const currentName = existingName.rows[0].field_value.trim();
        const nameParts = currentName.split(/\s+/);

        // Only combine if we currently have just a first name
        if (nameParts.length === 1) {
          const fullName = `${currentName} ${data.last_name.trim()}`;
          updates.push(
            updateCallerField(callerId, 'name', fullName, {
              sourceCallId: callId,
              sourceType: 'ai_extraction',
              confidence: 'stated'
            })
          );
          console.log(`   📝 Combined last name with existing first name: "${fullName}"`);
        }
      }
    } catch (err) {
      console.error('❌ Error combining last name:', err.message);
    }
  }
...
// ============================================
// ORGANIZATION NAME NORMALIZATION & FUZZY MATCHING
// ============================================
...
function normalizeOrgName(name) {
  ...
  return name
    .toLowerCase()
    .replace(/insurance\s*company/gi, '')
    .replace(/insurance/gi, '')
    .replace(/law\s*firm/gi, '')
    .replace(/law\s*office(s)?(\s*of)?/gi, '')
    .replace(/medical\s*office/gi, '')
    .replace(/medical\s*center/gi, '')
    .replace(/medical\s*department/gi, '')
    .replace(/medical/gi, '')
...
/**
 * Find similar organizations by name
 ...
 * @returns {array} Similar organizations sorted by similarity
 */
async function findSimilarOrganizations(name, agentId, threshold = 0.6) {
  if (!name || !agentId) return [];

  const normalizedInput = normalizeOrgName(name);
  if (normalizedInput.length < 2) return [];

  try {
    const result = await getPool().query(`
      SELECT id, name, type, primary_phone, total_calls
      FROM organizations
    ...
    for (const org of result.rows) {
      ...
      if (normalizedOrg === normalizedInput) {
        matches.push({ ...org, similarity: 1.0, matchType: 'exact_normalized' });
        continue;
      }

      // Check if one contains the other
      if (normalizedOrg.includes(normalizedInput) || normalizedInput.includes(normalizedOrg)) {
        matches.push({ ...org, similarity: 0.9, matchType: 'contains' });
        continue;
      }

      // Check similarity score
      const similarity = stringSimilarity(normalizedOrg, normalizedInput);
      if (similarity >= threshold) {
        matches.push({ ...org, similarity, matchType: 'fuzzy' });
      }
    }

    // Sort by similarity (highest first), then by total_calls (most calls first)
    matches.sort((a, b) => {
    ...
    console.error('❌ Error finding similar organizations:', error.message);
    return [];
  }
}
...
async function getOrCreateOrganization(orgData, agentId) {
  ...
  try {
    ...
    // Try to find by name using fuzzy matching
    const similarOrgs = await findSimilarOrganizations(name, agentId, 0.7);

    if (similarOrgs.length > 0) {
      // Use the best match (highest similarity)
      const bestMatch = similarOrgs[0];
      console.log(`   🔍 Fuzzy match found: "${name}" → "${bestMatch.name}" (${Math.round(bestMatch.similarity * 100)}% match)`);

      // Get full org record
      const orgResult = await getPool().query('SELECT * FROM organizations WHERE id = $1', [bestMatch.id]);
...
/**
 * Track that a professional contact called about a specific client
 * @param {number} contactId - The organization contact ID
 * @param {number} clientCallerId - The caller ID of the CourtLaw client
 * @param {object} options - Additional options
 * @returns {object} Association record
 */
async function trackContactClientAssociation(contactId, clientCallerId, options = {}) {
  if (!contactId || !clientCallerId) return null;

  const {
    organizationId = null,
    clientName = null,
    relationshipType = 'called_about',
    notes = null
  } = options;

  try {
    // Try to insert or update the association
    const result = await getPool().query(`
      INSERT INTO contact_client_associations
        (contact_id, client_caller_id, organization_id, client_name, relationship_type, notes)
      VALUES ($1, $2, $3, $4, $5, $6)
      ON CONFLICT (contact_id, client_caller_id) DO UPDATE SET
        last_call_date = NOW(),
        total_calls = contact_client_associations.total_calls + 1,
        client_name = COALESCE(EXCLUDED.client_name, contact_client_associations.client_name),
        notes = COALESCE(EXCLUDED.notes, contact_client_associations.notes),
        updated_at = NOW()
      RETURNING *
    `, [contactId, clientCallerId, organizationId, clientName, relationshipType, notes]);

    console.log(`   🔗 Tracked association: Contact ${contactId} → Client ${clientCallerId}`);
    return result.rows[0];
  } catch (error) {
    console.error('❌ Error tracking contact-client association:', error.message);
    return null;
  }
}

/**
 * Get all clients that contacts from an organization have called about
 * @param {number} organizationId - Organization ID
 * @returns {array} List of client associations
 */
async function getOrganizationLinkedClients(organizationId) {
  if (!organizationId) return [];

  try {
    // Get client mentions from contacts belonging to this organization
    const result = await getPool().query(`
      SELECT DISTINCT
        cca.id as association_id,
        cca.organization_contact_id,
        cca.client_name,
        cca.claim_number,
        cca.mention_count,
        cca.first_mentioned_call_id,
        cca.last_mentioned_call_id,
        cca.created_at as first_association_date,
        cca.updated_at as last_association_date,
      ...
      WHERE oc.organization_id = $1 AND cca.is_active = TRUE
      ORDER BY cca.updated_at DESC
    `, [organizationId]);

    return result.rows.map(row => ({
      associationId: row.association_id,
      contactId: row.organization_contact_id,
      contactName: row.contact_name,
      clientName: row.client_name || 'Unknown',
      claimNumber: row.claim_number,
      mentionCount: row.mention_count || 1,
      firstCallId: row.first_mentioned_call_id,
      lastCallId: row.last_mentioned_call_id,
      firstAssociationDate: row.first_association_date,
      lastAssociationDate: row.last_association_date
    }));
  } catch (error) {
    console.error('❌ Error getting organization linked clients:', error.message);
    return [];
  }
}

/**
 * Get full organization details including contacts and linked clients
 * @param {number} organizationId - Organization ID
 * @param {string} agentId - Agent ID for verification
 * @returns {object} Full organization details
 */
async function getOrganizationDetails(organizationId, agentIds) {
  if (!organizationId) return null;

  // Normalize agentIds to array
  const agentIdArray = Array.isArray(agentIds) ? agentIds : [agentIds];

  try {
    // Get organization info
    const orgResult = await getPool().query(`
      SELECT * FROM organizations WHERE id = $1 AND agent_id = ANY($2)
    `, [organizationId, agentIdArray]);
    ...
    const contactsResult = await getPool().query(`
      SELECT
        ...
        oc.direct_phone,
        oc.fax,
        oc.preferred_language,
        oc.total_calls,
        oc.first_call_date,
        oc.last_call_date,
        (SELECT COUNT(*) FROM contact_client_associations WHERE organization_contact_id = oc.id) as client_count
      FROM organization_contacts oc
      WHERE oc.organization_id = $1 AND oc.is_active = TRUE
      ORDER BY oc.total_calls DESC, oc.name ASC
    `, [organizationId]);

    // Get linked clients (clients mentioned by this organization's contacts)
    const linkedClients = await getOrganizationLinkedClients(organizationId);
    ...
    return {
      ...
      contacts: contactsResult.rows.map(c => ({
        ...
        preferredLanguage: c.preferred_language,
        totalCalls: c.total_calls,
        clientCount: parseInt(c.client_count) || 0,
        firstCallDate: c.first_call_date,
        lastCallDate: c.last_call_date
      })),
      linkedClients: linkedClients,
      contactCount: contactsResult.rows.length,
      linkedClientCount: linkedClients.length
    };
  } catch (error) {
...
/**
 * Find a CourtLaw client by name (for linking)
 * @param {string} clientName - Client name to search
 * @param {string} agentId - Agent ID
 * @returns {array} Matching clients
 */
async function findClientsByName(clientName, agentId) {
  if (!clientName || !agentId) return [];
  ...
  try {
    const result = await getPool().query(`
      ...
      WHERE c.agent_id = $1
        ...
        AND LOWER(cd.field_value) LIKE $2
    ...
    console.error('❌ Error finding clients by name:', error.message);
    return [];
  }
}
...
module.exports = {
  ...
  // Contact-client association functions
  trackContactClientAssociation,
  saveContactClientMention,  // New: saves when a professional mentions a CourtLaw client
  getOrganizationLinkedClients,
  getOrganizationDetails,
  findClientsByName
};

/**
 * Save a mention of a CourtLaw client by a professional caller
 * Uses the ACTUAL database schema (organization_contact_id, client_name, claim_number)
 * @param {number} contactId - Organization contact ID
 * @param {string} clientName - Name of the CourtLaw client being called about
 * @param {object} options - Additional options (claimNumber, callId, notes)
 * @returns {object} Association record
 */
async function saveContactClientMention(contactId, clientName, options = {}) {
  if (!contactId || !clientName) return null;

  const { claimNumber = null, callId = null, notes = null } = options;

  try {
    // Check if this association already exists
    const existing = await getPool().query(`
      SELECT id, mention_count, claim_number
      FROM contact_client_associations
      WHERE organization_contact_id = $1 AND LOWER(client_name) = LOWER($2)
    `, [contactId, clientName]);

    if (existing.rows.length > 0) {
      // Update existing - increment mention count, update claim if we have a new one
      const result = await getPool().query(`
        UPDATE contact_client_associations
        SET mention_count = mention_count + 1,
            claim_number = COALESCE($1, claim_number),
            last_mentioned_call_id = $2,
            notes = COALESCE($3, notes),
            updated_at = NOW()
        WHERE id = $4
        RETURNING *
      `, [claimNumber, callId, notes, existing.rows[0].id]);

      console.log(`   📋 Updated client mention: ${clientName} (${result.rows[0].mention_count} times)`);
      return { ...result.rows[0], isNew: false };
    } else {
      // Insert new association
      const result = await getPool().query(`
        INSERT INTO contact_client_associations
          (organization_contact_id, client_name, claim_number, first_mentioned_call_id, last_mentioned_call_id, mention_count, notes)
        VALUES ($1, $2, $3, $4, $4, 1, $5)
        RETURNING *
      `, [contactId, clientName, claimNumber, callId, notes]);

      console.log(`   📋 NEW client mention: ${clientName}${claimNumber ? ' [Claim: ' + claimNumber + ']' : ''}`);
      return { ...result.rows[0], isNew: true };
    }
  } catch (error) {
    console.error('❌ Error saving contact-client mention:', error.message);
    return null;
  }
}
