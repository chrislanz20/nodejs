  ...
  try {
    const result = await getPool().query(`
      SELECT DISTINCT
        ...
        cca.contact_id,
        cca.client_caller_id,
        cca.client_name,
        cca.relationship_type,
        cca.total_calls as association_calls,
        cca.first_call_date as first_association_date,
        cca.last_call_date as last_association_date,
        oc.name as contact_name,
        c.phone_number as client_phone,
        c.total_calls as client_total_calls,
        cd_name.field_value as client_name_from_profile,
        cd_email.field_value as client_email,
        cd_callback.field_value as client_callback_phone
      FROM contact_client_associations cca
      JOIN organization_contacts oc ON cca.contact_id = oc.id
      JOIN callers c ON cca.client_caller_id = c.id
      LEFT JOIN caller_details cd_name ON c.id = cd_name.caller_id
        AND cd_name.field_name = 'name' AND cd_name.valid_until IS NULL
      LEFT JOIN caller_details cd_email ON c.id = cd_email.caller_id
        AND cd_email.field_name = 'email' AND cd_email.valid_until IS NULL
      LEFT JOIN caller_details cd_callback ON c.id = cd_callback.caller_id
        AND cd_callback.field_name = 'callback_phone' AND cd_callback.valid_until IS NULL
      WHERE cca.organization_id = $1 OR oc.organization_id = $1
      ORDER BY cca.last_call_date DESC
    `, [organizationId]);
    ...
    return result.rows.map(row => ({
      ...
      clientCallerId: row.client_caller_id,
      clientName: row.client_name || row.client_name_from_profile || 'Unknown',
      clientPhone: row.client_phone,
      clientEmail: row.client_email,
      clientCallbackPhone: row.client_callback_phone,
      relationshipType: row.relationship_type,
      associationCalls: row.association_calls,
      clientTotalCalls: row.client_total_calls,
  ...
  try {
    ...
    const contactsResult = await getPool().query(`
      SELECT
        ...
        oc.first_call_date,
        oc.last_call_date,
        (SELECT COUNT(*) FROM contact_client_associations WHERE contact_id = oc.id) as client_count
    ...
    // Also get clients from caller records that might not have associations yet
    // These are CourtLaw clients that were mentioned by this org's contacts
    const additionalClientsResult = await getPool().query(`
      SELECT DISTINCT
        c.id as client_id,
        c.phone_number,
        c.total_calls,
        c.last_call_date,
        cd_name.field_value as name,
        cd_email.field_value as email
      FROM callers c
      LEFT JOIN caller_details cd_name ON c.id = cd_name.caller_id
        AND cd_name.field_name = 'name' AND cd_name.valid_until IS NULL
      LEFT JOIN caller_details cd_email ON c.id = cd_email.caller_id
      ...
      WHERE c.agent_id = ANY($1)
        ...
        AND c.id IN (
          SELECT DISTINCT client_caller_id FROM contact_client_associations
          WHERE organization_id = $2
        )
      ORDER BY c.last_call_date DESC
    `, [agentIdArray, organizationId]);
