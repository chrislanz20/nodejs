...
// Staff/attorney names that should NEVER appear as caller/client names
// These are CourtLaw employees
const STAFF_NAMES = [
...
/**
 * Check if a name matches a staff member
 */
function isStaffName(name) {
  if (!name || typeof name !== 'string') return false;
  const lowerName = name.toLowerCase().trim();
  return STAFF_NAMES.some(staff => lowerName === staff || lowerName.includes(staff));
}

/**
 * Clean staff names from caller data - returns 'Unknown' if name is a staff member
 */
function cleanCallerName(name) {
  if (isStaffName(name)) {
    console.log(`⚠️ Filtered out staff name from caller data: "${name}"`);
    return 'Unknown';
  }
  return name;
}
...
const rawTemplates = {
  ...
  insurance: fs.readFileSync(path.join(templatesDir, 'email_insurance_raw.html'), 'utf8'),
...
/**
 * Convert N8N/Handlebars template syntax to JavaScript
 * Handles:
 * - {{ $json.fieldName }} → data value or 'Not provided'
 * - {{ $json['Field Name'] }} → data value or 'Not provided'
 * - {{ fieldName }} → data value or empty string (for optional fields)
 * - {{#if fieldName}}...{{/if}} → show content only if field has value
 */
function convertN8NTemplate(template, data) {
  ...
  // Handle {{#if fieldName}}...{{/if}} conditionals FIRST
  // Process iteratively to handle NESTED conditionals (innermost first)
  ...
  while (result !== previousResult && iterations < maxIterations) {
    ...
    // Match innermost {{#if}}...{{/if}} blocks (those that don't contain other {{#if}})
    result = result.replace(/\{\{#if\s+(\w+)\}\}((?:(?!\{\{#if)[\s\S])*?)\{\{\/if\}\}/g, (match, fieldName, content) => {
      const value = data[fieldName];
      // Show content only if field has a truthy value (not empty string, null, undefined)
  ...
  // Replace bare {{ fieldName }} (without $json) - used in New Lead template
  // These are optional fields, so return empty string instead of 'Not provided'
  result = result.replace(/\{\{\s*(\w+)\s*\}\}/g, (match, fieldName) => {
...
function generateAttorneyEmail(callData) {
  const data = {
    Name: cleanCallerName(callData.name) || 'Unknown',
    ...
    // Use call_summary first (3-5 sentence AI summary) for detailed context
    Purpose: callData.call_summary || callData.purpose || callData.call_purpose || callData.incident_description || 'Attorney inquiry'
  };
...
function generateMedicalEmail(callData) {
  const data = {
    ...
    // Use call_summary first (3-5 sentence AI summary) for detailed context
    Purpose: callData.call_summary || callData.purpose || callData.call_purpose || callData.incident_description || 'Medical inquiry'
  };
 ...
 */
function generateInsuranceEmail(callData) {
  const data = {
    Name: cleanCallerName(callData.name) || 'Unknown',
    Phone: callData.phone || callData.phone_number || 'Not provided',
    InboundPhoneNum: callData.from_number || callData.to_number || 'Not available',
    ...
    client_name: callData.client_name || callData.case_name || 'Not provided',
    // Use call_summary first (3-5 sentence AI summary) for detailed context
    Purpose: callData.call_summary || callData.purpose || callData.call_purpose || callData.incident_description || 'Insurance inquiry'
  };

  return {
    subject: 'Inbound Call Alert — Insurance',
    html: convertN8NTemplate(rawTemplates.insurance, data)
  };
}
...
function generateOtherEmail(callData) {
  const data = {
    ...
    Email: callData.email || callData.email_address || 'Not provided',
    // Professional AI summary (3-5 sentences with filtered content)
    call_summary: callData.call_summary || callData.purpose || callData.incident_description || 'General inquiry',
    // Inquiry Details - use call_summary first for detailed context
    Purpose: callData.call_summary || callData.purpose || callData.call_purpose || 'Not provided',
    representing_who: callData.representing_who || callData.who_representing || 'Not provided',
    client_name: callData.client_name || callData.case_name || 'Not provided',
...
function generateNewLeadEmail(callData) {
  const rawName = callData.name || (callData.first_name && callData.last_name ? callData.first_name + ' ' + callData.last_name : null);
  const data = {
    name: cleanCallerName(rawName) || 'Unknown',
    phone: callData.phone || callData.phone_number || 'Not provided',
...
function generateExistingClientEmail(callData) {
  ...
  let returningCallerNote = '';
  if (callData.is_returning_caller && callData.total_calls > 1) {
    returningCallerNote = `<p style="color: #2563eb; font-weight: bold;">📞 Returning Caller (Call #${callData.total_calls})</p>`;
    if (callData.associated_cases && callData.associated_cases.length > 0) {
      returningCallerNote += `<p style="color: #059669;">📁 Associated Cases: ${callData.associated_cases.join(', ')}</p>`;
    }
  }

  const rawName = callData.name || (callData.first_name + ' ' + callData.last_name) || null;
  const data = {
    Name: cleanCallerName(rawName) || 'Unknown',
    Phone: callData.phone || callData.phone_number || 'Not provided',
    InboundPhoneNum: callData.from_number || callData.to_number || 'Not available',
    Email: callData.email || callData.email_address || 'Not provided',
    // Use call_summary first (3-5 sentence AI summary) for detailed context
    Purpose: callData.call_summary || callData.purpose || callData.call_purpose || callData.incident_description || 'Client follow-up',
    representing_who: callData.representing_who || callData.who_representing || 'Not provided',
    client_name: callData.client_name || callData.case_name || 'Not provided',
    claim_num: callData.claim_num || callData.claim_number || 'Not provided',
    returning_caller_note: returningCallerNote
  };
...
function generateEmail(category, callData) {
  const categoryMap = {
    ...
    'Insurance': generateInsuranceEmail,
