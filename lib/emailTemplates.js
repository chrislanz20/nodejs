...
function generateSMS(callData) {
  ...
  // Add returning caller indicator
  let returningNote = '';
  if (callData.is_returning_caller && callData.total_calls > 1) {
    returningNote = `\n\n📞 RETURNING CALLER (Call #${callData.total_calls})`;
    if (callData.associated_cases && callData.associated_cases.length > 0) {
      returningNote += `\nPrior Cases: ${callData.associated_cases.join(', ')}`;
    }
  }

  return `New Inbound Lead${caseType}${returningNote}
