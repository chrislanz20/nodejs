...
// ============================================
// TEST MODE - TEMPORARY FOR CONVERSATION FLOW TESTING
// Set to true to only send notifications to TEST_EMAIL
// REMEMBER TO SET BACK TO false WHEN DONE TESTING!
// ============================================
const TEST_MODE = false;
const TEST_EMAIL = '17lanzch@gmail.com';
// ============================================
...
async function getNotificationRecipients(agentId, category) {
  try {
    ...
    const result = await pool.query(`
      SELECT
        nr.id, nr.name, nr.email, nr.phone, nr.ghl_contact_id, nr.active, nr.is_primary,
...
/**
 * Send grouped email via Resend API (with retry)
 * Sends one email with TO recipients and BCC recipients to save on email quota
 * @param {Array} toRecipients - Array of {name, email} for TO field
 * @param {Array} bccRecipients - Array of {name, email} for BCC field
 * @param {string} subject - Email subject
 * @param {string} htmlContent - Email HTML content
 * @param {string} fromName - Sender name (defaults to 'SaveYa Tech Notifications')
 */
async function sendGroupEmailViaResend(toRecipients, bccRecipients, subject, htmlContent, fromName = 'SaveYa Tech Notifications') {
  let lastError = null;

  // Extract just email addresses for the API
  const toEmails = toRecipients.map(r => r.email);
  const bccEmails = bccRecipients.map(r => r.email);

  // Log what we're sending
  console.log(`   📧 Sending grouped email:`);
  console.log(`      TO: ${toRecipients.map(r => r.name).join(', ')}`);
  if (bccRecipients.length > 0) {
    console.log(`      BCC: ${bccRecipients.map(r => r.name).join(', ')}`);
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // Only add BCC if there are BCC recipients
      ...
      return {
        ...
        totalRecipients: toRecipients.length + bccRecipients.length
      };
    } catch (error) {
      lastError = error;
      ...
      if (isRetryableError(error) && attempt < MAX_RETRIES) {
        const delay = INITIAL_RETRY_DELAY * Math.pow(2, attempt - 1);
        console.log(`   ⚠️ Grouped email attempt ${attempt} failed (${error.message}), retrying in ${delay}ms...`);
        await sleep(delay);
      } else {
        break;
      }
  ...
  console.error(`❌ Failed to send grouped email after ${MAX_RETRIES} attempts:`, lastError.message);
  return {
    success: false,
    error: lastError.message,
    toRecipients: toRecipients,
...
async function shouldSendOtherEmail(callData) {
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      model: 'claude-opus-4-5-20251101', // Claude Opus 4.5 - maximum accuracy
      max_tokens: 50,
      messages: [{
        role: 'user',
...
async function sendNotifications(agentId, category, callData) {
  ...
  // TEST MODE FILTER - Only send to test email, skip all SMS
  if (TEST_MODE) {
    console.log(`⚠️  TEST MODE ACTIVE - Only sending notifications to ${TEST_EMAIL}`);
    console.log(`⚠️  SMS notifications are DISABLED in test mode`);
  }
  ...
  if (dbRecipients.length > 0) {
    ...
    let emailRecipients = dbRecipients.filter(r => r.email_enabled && r.email);
    // Filter recipients who should receive SMS for this category (need GHL contact for SMS)
    let smsRecipients = dbRecipients.filter(r => r.sms_enabled && r.ghl_contact_id && r.phone);

    // TEST MODE: Filter to only test email and disable SMS
    if (TEST_MODE) {
      emailRecipients = emailRecipients.filter(r => r.email === TEST_EMAIL);
      if (emailRecipients.length === 0) {
        // Add test email as recipient if not in list
        emailRecipients = [{ name: 'Test User', email: TEST_EMAIL, is_primary: true }];
      }
      smsRecipients = []; // Disable all SMS in test mode
    }
    ...
    // Send emails via Resend using grouped approach (1 email with TO + BCC)
    if (emailRecipients.length > 0) {
      // Separate into TO (primary) and BCC (non-primary) recipients
      const toRecipients = emailRecipients.filter(r => r.is_primary).map(r => ({ name: r.name, email: r.email }));
      const bccRecipients = emailRecipients.filter(r => !r.is_primary).map(r => ({ name: r.name, email: r.email }));

      // If no primary recipients, use first recipient as TO
      if (toRecipients.length === 0 && emailRecipients.length > 0) {
        toRecipients.push({ name: emailRecipients[0].name, email: emailRecipients[0].email });
        // Remove from BCC if it was there
        const idx = bccRecipients.findIndex(r => r.email === emailRecipients[0].email);
        if (idx > -1) bccRecipients.splice(idx, 1);
      }

      // Try grouped email first (saves email quota)
      const groupedResult = await sendGroupEmailViaResend(
        toRecipients,
        bccRecipients,
        emailData.subject,
        emailData.html,
        'SaveYa Tech Notifications'
      );

      if (groupedResult.success) {
        console.log(`✅ Grouped email sent successfully (${groupedResult.totalRecipients} recipients, 1 email used)`);
        results.emails_sent.push({
          type: 'grouped',
          messageId: groupedResult.messageId,
          to: toRecipients.map(r => r.email),
          bcc: bccRecipients.map(r => r.email),
          totalRecipients: groupedResult.totalRecipients
        });
      } else {
        // FALLBACK: If grouped email fails, send individual emails
        console.log(`⚠️  Grouped email failed, falling back to individual emails...`);
    ...
    let emailAddresses = getEmailRecipients(agentId, category);

    // TEST MODE: Filter to only test email
    if (TEST_MODE) {
      emailAddresses = emailAddresses.filter(e => e === TEST_EMAIL);
      if (emailAddresses.length === 0) {
        emailAddresses = [TEST_EMAIL]; // Always send to test email
      }
    }
    ...
    // Skip SMS entirely in TEST_MODE
    if (category === 'New Lead' && !TEST_MODE) {
      ...
      console.log(`ℹ️  No SMS sent (TEST MODE - SMS disabled)`);
    } else {
