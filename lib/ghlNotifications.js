...
async function sendNotifications(agentId, category, callData) {
  ...
  // Send emails to all contacts EXCEPT Existing Client (they get SMS only)
  if (category !== 'Existing Client') {
    ...
    console.log(`ℹ️  No email sent for Existing Client (SMS only)`);
  }

  // Send SMS for New Leads AND Existing Clients
  if (category === 'New Lead' || category === 'Existing Client') {
    const smsType = category === 'New Lead' ? 'New Lead' : 'Existing Client';
    console.log(`📱 Generating SMS for ${smsType}...`);
    ...
    console.log(`ℹ️  No SMS sent (category: ${category} - SMS only sent for New Leads and Existing Clients)`);
  }
