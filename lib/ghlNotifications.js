    ...
    try {
      const response = await axios.post(
        `${GHL_BASE_URL}/conversations/messages`,
        {
          ...
          contactId: contactId,
          emailFrom: 'notifications@saveyatech.com',
          subject: subject,
          html: htmlContent
        },
        {
...
async function sendNotifications(agentId, category, callData) {
  ...
  if (dbRecipients.length > 0) {
    ...
    // Filter recipients who should receive emails for this category
    const emailRecipients = dbRecipients.filter(r => r.email_enabled && r.ghl_contact_id);
    // Filter recipients who should receive SMS for this category
    const smsRecipients = dbRecipients.filter(r => r.sms_enabled && r.ghl_contact_id && r.phone);

    console.log(`   - ${emailRecipients.length} recipient(s) will receive email`);
    console.log(`   - ${smsRecipients.length} recipient(s) will receive SMS`);

    // Send emails to recipients who have email enabled for this category
    ...
    // Send SMS only for New Leads (fallback behavior)
