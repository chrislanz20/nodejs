...
// Gemini for claim number extraction (more accurate for consecutive digits)
...
function extractFromAgentReadback(text) {
  ...
  // Handle letter prefixes followed by dash-separated digits (C-5-3-3-1-7-2-3-1-0)
  // This captures patterns like "C-5-3-3" where a letter prefix precedes digits
  const letterDashPattern = /([A-Z])-(\d(?:-\d)+)/gi;
  let letterDashMatch;
  while ((letterDashMatch = letterDashPattern.exec(cleanedText)) !== null) {
    result += letterDashMatch[1].toUpperCase() + letterDashMatch[2].replace(/-/g, '');
  }

  // Handle dash-separated digits WITHOUT letter prefix (2-8-3-9-9-7-5-1)
  // Use negative lookbehind to avoid matching digits already captured with letter prefix
  const dashPattern = /(?<![A-Z]-)(\d(?:-\d)+)/g;
  let dashMatch;
  ...
  // BUT: Don't match if followed by more dash-separated digits (those are handled by dashPattern)
  // e.g., "dash 0-0-0-1" should NOT match here - the "0-0-0-1" is handled separately
  const dashEndPattern = /dash\s+(one|two|three|four|five|six|seven|eight|nine|zero|oh|\d)(?!-\d)/gi;
  let dashEndMatch;
...
/**
 * Extract claim number using Gemini 2.5 Pro API
 * Gemini is more accurate than Opus for counting consecutive digits and cheaper than Sonnet
 *
 * @param {Array} transcriptArray - Array of {role, content} messages
 * @returns {Promise<string|null>} - Extracted claim number or null
 */
async function extractClaimNumberWithGemini(transcriptArray) {
  if (!Array.isArray(transcriptArray) || transcriptArray.length === 0) {
    return null;
  }

  // Check if caller explicitly said they don't have a claim number
  if (callerSaidNoClaimNumber(transcriptArray)) {
    console.log('   ⏭️ Caller explicitly said no claim number - skipping Gemini extraction');
    return null;
  }
  ...
  const prompt = `Extract the claim number or policy number from this call transcript.

CRITICAL: Count the digits EXACTLY as spoken. The agent confirms the number by reading back each digit as a word. Count each word carefully:
- "zero" = 0
- "one" = 1
- "two" = 2
- "three" = 3
...
IMPORTANT:
- Count EVERY digit word spoken, including consecutive zeros
- The agent's readback is the most reliable source
- Look for letters like "C" or "S" at the beginning (e.g., "C-5-3-3" = "C533")
- Include any prefix letters in your answer

Return ONLY the claim/policy number as digits (with any prefix letters). No explanation, no extra text. Just the number.

If no claim number is mentioned or the caller says they don't have one, return exactly: NO_CLAIM_NUMBER
...
${transcriptText}`;
  ...
  try {
    ...
    if (extracted === 'NO_CLAIM_NUMBER' || !extracted) {
      return null;
    }

    // Clean up the result - remove any non-alphanumeric characters except dashes
    const cleaned = extracted.replace(/[^a-zA-Z0-9-]/g, '').replace(/-+/g, '');
    console.log(`   🔍 Gemini 2.5 Pro extracted claim number: ${cleaned}`);
    return cleaned;

  } catch (error) {
    console.error('   ❌ Gemini claim extraction failed:', error.message);
    return null;
  }
}

/**
 * Extract claim number using DETERMINISTIC programmatic extraction (fallback)
 *
 * Strategy:
 ...
 * @returns {Promise<string|null>} - Extracted claim number or null
 */
async function extractClaimNumberProgrammatic(transcriptArray) {
  if (!Array.isArray(transcriptArray) || transcriptArray.length === 0) {
    return null;
  }
...
ALL CALLS:
- name: Full name of caller (first AND last)
- last_name: Last name ONLY if agent specifically asked for just the last name (to update records)
...
Return format (JSON only - _thinking MUST be first):
{
  ...
  "name": "string or null",
  "last_name": "string or null (only if agent asked specifically for last name)",
  ...
  "doctor_visit": "string or null"
    ...
    // PRIMARY: Use Gemini 2.5 Pro - more accurate than Opus for counting consecutive digits, cheaper than Sonnet
    // FALLBACK 1: Regex extraction from confirmed readback
    // FALLBACK 2: Programmatic word-to-digit conversion
    if (Array.isArray(transcript)) {
      // PRIMARY: Use Gemini 2.5 Pro for claim number (better at counting consecutive digits)
      const geminiClaimNumber = await extractClaimNumberWithGemini(transcript);
      if (geminiClaimNumber) {
        extracted.claim_number = geminiClaimNumber;
        console.log(`   ✅ Claim number from Gemini 2.5 Pro: ${extracted.claim_number}`);
      } else {
        // FALLBACK 1: Try regex extraction from confirmed readback
        ...
        if (confirmedClaimNumber) {
          ...
          console.log(`   ✅ Claim number from fallback (regex): ${extracted.claim_number}`);
        }
        // FALLBACK 2: Programmatic extraction as last resort
        if (!extracted.claim_number) {
          const programmaticClaimNumber = await extractClaimNumberProgrammatic(transcript);
          if (programmaticClaimNumber) {
            extracted.claim_number = programmaticClaimNumber;
            console.log(`   ✅ Claim number from fallback (programmatic): ${extracted.claim_number}`);
          }
        }
      }
    }
