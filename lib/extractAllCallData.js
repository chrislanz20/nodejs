 ...
 * 2. Find ALL claim number exchanges in the transcript
 * 3. Identify the LAST CONFIRMED claim number (agent reads back, user confirms)
 * 4. Use the USER's clear digit-by-digit statement (most reliable)
 * 5. Send to Claude Sonnet 4 for accurate conversion
 *
...
async function extractClaimNumberWithSonnet(transcriptArray) {
  ...
  for (let i = 0; i < transcriptArray.length; i++) {
    ...
    if (msg.role === 'agent') {
      ...
      // Check if agent is reading back/confirming A CLAIM NUMBER (not a name)
      const isReadingBack = (
        (content.includes('claim number') || content.includes('file number')) &&
        !isNameSpelling(msg.content, transcriptArray, i) &&
      ...
      // But make sure it's not actually about a name
      ...
      // Use the AGENT's readback because:
      // 1. Agent may have combined split user messages (e.g., user said "three two one" then "zero one")
      // 2. Agent's readback is already confirmed accurate by the user
      if (isReadingBack && i + 1 < transcriptArray.length) {
        ...
        if (userResponse.role === 'user' && isConfirmation(userResponse.content)) {
          lastConfirmedExchange = {
            userStatement: null,  // Not used when agent readback is confirmed
            agentReadback: msg.content,
            confirmed: true,
            useAgentReadback: true  // Use agent's readback - user confirmed it's correct
          };
        }
      }
  ...
  // Use programmatic conversion for 100% accuracy
  const digitMap = {
    'zero': '0', 'oh': '0',
  ...
  // Split into words and convert each one
  const words = textToExtract.toLowerCase().split(/[\s.,;:!?]+/).filter(w => w);
  let result = '';
  ...
  for (const word of words) {
    ...
    // Check for "X zeros" pattern (e.g., word is a number followed by next word being "zeros")
    // This is handled by looking ahead
    // Skip filler words like "the", "is", "it's", "actually", etc.
  }

  // Handle "X zeros" patterns (e.g., "six zeros" should be 000000)
  // Re-process looking for number + "zeros" pattern
  let finalResult = '';
  for (let i = 0; i < words.length; i++) {
    const word = words[i];
    const nextWord = words[i + 1];

    // Check for "X zeros" pattern
    if (nextWord && (nextWord === 'zeros' || nextWord === 'zeroes')) {
      const count = digitMap[word];
      if (count && parseInt(count) > 0 && parseInt(count) <= 9) {
        finalResult += '0'.repeat(parseInt(count));
        i++; // Skip the "zeros" word
        continue;
      }
    }
    ...
    // Regular conversion
    if (digitMap[word]) {
      finalResult += digitMap[word];
    } else if (/^[a-z]$/i.test(word)) {
      finalResult += word.toUpperCase();
    } else if (/^\d+$/.test(word)) {
      finalResult += word;
    }
    // Skip other words (filler words, "zeros" when not part of pattern, etc.)
  }

  // Use finalResult if we processed "X zeros" patterns, otherwise use result
  const claimNumber = finalResult || result;
  ...
  // - Must contain at least one digit (pure letter strings are likely names, not claim numbers)
  const hasDigit = /\d/.test(claimNumber);

  if (claimNumber && claimNumber.length >= 5 && claimNumber.length <= 30 && hasDigit) {
    console.log(`   ✅ Extracted claim number: ${claimNumber}`);
    return claimNumber.toUpperCase();
  } else {
    if (claimNumber && claimNumber.length > 0) {
      const reason = claimNumber.length < 5 ? 'too short' : (!hasDigit ? 'no digits' : 'invalid');
      console.log(`   ⚠️ Rejected "${claimNumber}" (${reason}) from: "${textToExtract}"`);
    }
    return null;
  }
}
