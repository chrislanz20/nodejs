...
/**
 * Extract spoken digits from a string and convert to number
 * @param {string} text - Text containing spoken digits
...
function extractSpokenDigits(text) {
  ...
  for (const word of words) {
    ...
    if (DIGIT_WORDS[cleanWord] !== undefined) {
      result += DIGIT_WORDS[cleanWord];
    }
  }
...
/**
 * Validate and correct claim number by finding the spoken version in transcript
 * @param {string} claimNumber - The extracted claim number
 * @param {string} transcript - The full transcript text
 * @returns {string} Validated/corrected claim number
 */
function validateClaimNumber(claimNumber, transcript) {
  if (!claimNumber || !transcript) return claimNumber;

  // Extract any letter prefix (like LA, NJ)
  const prefixMatch = claimNumber.match(/^([A-Za-z]+)/);
  const prefix = prefixMatch ? prefixMatch[1].toUpperCase() : '';
  const extractedDigits = claimNumber.replace(/[^0-9]/g, '');

  // Look for patterns where someone reads out a file/claim number
  // Pattern: "file number is [letters] [spoken digits]" or "claim number [spoken digits]"
  const patterns = [
    /(?:file|claim|case)\s*(?:number|#)?\s*(?:is\s*)?([a-z]{1,3})?\s*((?:(?:zero|one|two|three|four|five|six|seven|eight|nine|oh|o)\s*)+)/gi,
    /([a-z])\s*(?:as in|like)\s*\w+[,\s]+([a-z])\s*(?:as in|like)\s*\w+[,\s]+((?:(?:zero|one|two|three|four|five|six|seven|eight|nine|oh|o)[,\s]*)+)/gi
  ];
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(transcript)) !== null) {
      ...
      const spokenDigits = extractSpokenDigits(fullMatch);

      // If the spoken digits length differs significantly from extracted, use spoken version
      if (spokenDigits.length > 0 && spokenDigits.length !== extractedDigits.length) {
        // Check if the spoken version makes more sense
        if (Math.abs(spokenDigits.length - extractedDigits.length) <= 3) {
          console.log(`   ⚠️ Digit count mismatch: extracted ${extractedDigits.length} digits, spoken ${spokenDigits.length} digits`);
  ...
  // Also look for confirmed numbers (agent reads back, caller says yes)
  const confirmPattern = /(?:let me (?:make sure|confirm)|is it|did i get)\s*[^.]*?([a-z])\s*(?:as in|like)\s*\w+[,\s]+([a-z])\s*(?:as in|like)\s*\w+[,\s]*((?:(?:zero|one|two|three|four|five|six|seven|eight|nine|oh|o)[,\s]*)+)[^.]*?\?[\s\S]{0,100}(?:yes|yeah|correct|right|that's right)/gi;

  let confirmMatch;
  while ((confirmMatch = confirmPattern.exec(transcript)) !== null) {
    const confirmedDigits = extractSpokenDigits(confirmMatch[0]);
    if (confirmedDigits.length > 0) {
      // Extract the letter prefix from confirmation
      const letter1 = confirmMatch[1] ? confirmMatch[1].toUpperCase() : '';
      const letter2 = confirmMatch[2] ? confirmMatch[2].toUpperCase() : '';
      const confirmedPrefix = letter1 + letter2;

      if (confirmedDigits.length !== extractedDigits.length) {
        console.log(`   ⚠️ Using confirmed number: ${confirmedPrefix}${confirmedDigits} (was ${claimNumber})`);
        return confirmedPrefix + confirmedDigits;
      }
    }
  }
...
Return format (JSON only, include all fields even if null):
{
  ...
  "doctor_visit": "string or null"
    ...
    // Post-process: Validate claim number by counting spoken digits
    if (extracted.claim_number && transcriptText) {
      const validated = validateClaimNumber(extracted.claim_number, transcriptText);
      if (validated !== extracted.claim_number) {
        console.log(`   🔧 Corrected claim number: ${extracted.claim_number} → ${validated}`);
        extracted.claim_number = validated;
      }
    }
