...
Return format (JSON only, include all fields even if null):
{
  ...
  "doctor_visit": "string or null"
    ...
    // Strategy: Trust Haiku's extraction from the agent's confirmed readback when available.
    // The agent reads back the number digit-by-digit ("8 7 9 6 5 9 6 6 1 0 0 0 0 0 0 1") and
    // the user confirms - Haiku can read this directly. Only use programmatic extraction as fallback.
    if (Array.isArray(transcript)) {
      // If Haiku extracted a claim number that looks valid, trust it
      // (Haiku reads the agent's numeric readback which is already in digit form)
      if (extracted.claim_number && extracted.claim_number.length >= 5 && /\d/.test(extracted.claim_number)) {
        // Clean up the claim number - remove dashes and spaces
        extracted.claim_number = extracted.claim_number.replace(/[-\s]/g, '');
        console.log(`   ✅ Extracted claim number: ${extracted.claim_number}`);
      } else {
        // Fallback: try programmatic extraction from user's spoken words
        const programmaticClaimNumber = await extractClaimNumberWithSonnet(transcript);
        if (programmaticClaimNumber) {
          console.log(`   ✅ Extracted claim number (fallback): ${programmaticClaimNumber}`);
