...
Return format (JSON only - _thinking MUST be first):
{
  ...
  "doctor_visit": "string or null"
    ...
    // PRIMARY: Trust Opus 4.5's extraction - it reads the transcript directly
    // FALLBACK: Only use regex extraction if Opus returned nothing valid
    if (Array.isArray(transcript)) {
      const opusClaimNumber = extracted.claim_number;
      const isValidOpusExtraction = opusClaimNumber &&
                                     opusClaimNumber.length >= 5 &&
                                     /\d/.test(opusClaimNumber) &&
                                     opusClaimNumber.length <= 30;

      if (isValidOpusExtraction) {
        // Clean up formatting (remove dashes/spaces) but trust Opus's value
        extracted.claim_number = opusClaimNumber.replace(/[-\s]/g, '');
        console.log(`   ✅ Claim number from Opus: ${extracted.claim_number}`);
      } else {
        // FALLBACK: Try regex extraction only if Opus failed
        ...
        // If still nothing, try programmatic extraction as last resort
