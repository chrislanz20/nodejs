...
function convertSpokenToDigits(spokenText) {
  ...
  const words = spokenText.toLowerCase().replace(/[^a-z\s-]/g, '').split(/\s+/);
  return words.map(w => WORD_TO_DIGIT[w]).filter(d => d !== undefined).join('');
}

/**
 * Extract claim number from transcript using programmatic digit conversion
 * This finds where the user provides a claim/file number and converts it accurately
...
function extractClaimNumberProgrammatically(transcriptArray) {
  ...
  for (let i = 0; i < transcriptArray.length; i++) {
    ...
    if (msg.role === 'agent') {
      ...
      if (content.includes('claim number') ||
