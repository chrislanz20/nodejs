...
/**
 * Extract claim number using Claude Sonnet 4 API
 * Sonnet is more accurate than Opus for counting consecutive digits
 *
 * @param {Array} transcriptArray - Array of {role, content} messages
 * @returns {Promise<string|null>} - Extracted claim number or null
 */
async function extractClaimNumberWithSonnetAPI(transcriptArray) {
  ...
  if (callerSaidNoClaimNumber(transcriptArray)) {
    console.log('   ⏭️ Caller explicitly said no claim number - skipping Sonnet extraction');
    return null;
  }
...
CRITICAL INSTRUCTION: The agent confirms the claim number by reading back each digit as a word. You MUST count EXACTLY how many times each digit word appears in sequence.

For example, if the agent says "one two one zero zero zero zero zero zero one one one three nine five zero", count each word:
1. "one" → 1
2. "two" → 2
3. "one" → 1
4. "zero" → 0
...
That would be exactly 16 digits: 1210000001113950
...
${transcriptText}`;
  ...
  try {
    ...
    console.log(`   🔍 Sonnet 4 extracted claim number: ${cleaned}`);
    return cleaned;

  } catch (error) {
    console.error('   ❌ Sonnet claim extraction failed:', error.message);
    return null;
  }
}
...
Return format (JSON only - _thinking MUST be first):
{
  ...
  "doctor_visit": "string or null"
    ...
    // PRIMARY: Use Sonnet 4 API - more accurate than Opus for counting consecutive digits
    // FALLBACK 1: Regex extraction from confirmed readback
    // FALLBACK 2: Programmatic word-to-digit conversion
    if (Array.isArray(transcript)) {
      // PRIMARY: Use Sonnet 4 for claim number (better at counting consecutive digits)
      const sonnetClaimNumber = await extractClaimNumberWithSonnetAPI(transcript);
      if (sonnetClaimNumber) {
        extracted.claim_number = sonnetClaimNumber;
        console.log(`   ✅ Claim number from Sonnet 4: ${extracted.claim_number}`);
      } else {
