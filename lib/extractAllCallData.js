...
3. For claim numbers / file numbers - CRITICAL ACCURACY REQUIRED:
   ...
   - When converting spoken digits to numbers, COUNT EACH DIGIT INDIVIDUALLY
   - "zero zero zero zero" = exactly 4 zeros (0000), NOT 5 zeros
   - "zero zero zero zero zero" = exactly 5 zeros (00000)
   - Count by going through each spoken word one by one
   - Include any letter prefixes (like "LA" or "NJ") exactly as stated
   - The FINAL number of digits must EXACTLY match the number of digits spoken in the transcript
   - Double-check your count before returning the claim number
