...
function convertSpokenToDigits(spokenText) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // Skip the word "a" (as in "ends with a 1" - "a" is an article, not a digit)
    if (cleanToken === 'a') {
      continue;
    }

    // Single letter at the START (like "l a" for "LA") - for alphanumeric claim numbers
    // Only allow letters if they appear at the very beginning before any digits
    if (/^[a-z]$/.test(cleanToken) && parts.length === 0) {
