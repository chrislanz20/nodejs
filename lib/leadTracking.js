...
async function trackLead(callId, agentId, category, callData) {
  ...
  try {
      ...
      if (category === 'New Lead') {
        ...
        const result = await pool.query(
          ...
          [
            ...
            agentId,
            phoneNumber,
            callData.name || callData.first_name + ' ' + callData.last_name || null,
    ...
    if (existingLead.category === 'New Lead' && category === 'Existing Client') {
      ...
      console.log(`🎉 CONVERSION DETECTED! ${existingLead.name || phoneNumber} called back as Existing Client`);

      return {
        lead: { ...existingLead, conversion_detected: true, conversion_call_id: callId },
        isNewLead: false,
        conversionDetected: true
      };
    }
